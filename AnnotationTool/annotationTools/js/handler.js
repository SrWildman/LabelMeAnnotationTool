...
function handler() {
  ...
  this.SubmitEditLabelTag = function (idx) {
    ...
    // object name
    old_name = LMgetObjectField(LM_xml, idx, 'name');
    if (document.getElementById('objEnter')) new_name = RemoveSpecialChars(document.getElementById('objEnter').value);
    else new_name = RemoveSpecialChars(adjust_objEnter);
    ...
    // Set fields:
    LMsetTagField(LM_xml, obj_ndx, "name", new_name);
    LMsetTagField(LM_xml, obj_ndx, "automatic", "0");
  ...
  this.EditBubbleDeleteButtonTag = function (idx) {
    ...
    // Insert data for server logfile:
    old_name = LMgetObjectField(LM_xml, select_anno, 'name');
    new_name = old_name;
    WriteLogMsg('*Deleting_object');
    InsertServerLogData('cpts_not_modified');

    // Set <deleted> in LM_xml:
    LMsetTagField(LM_xml, idx, "deleted", "1");
  ...
  this.TagLinkClick = function (idx) {
    if (adjust_event) return;
    if (video_mode && LMgetTagField(LM_xml, idx, 'x', oVP.getcurrentFrame()).length == 0) {
      // get frame that is closest

      var frames = LMgetTagField(LM_xml, idx, 't');
      var id1 = -1;
      var id2 = frames.length;
      var i = 0;
      while (i < frames.length) {
    ...
    if (active_canvas == REST_CANVAS) StartTagEdit(idx, null);
    else if (active_canvas == SELECTED_CANVAS) {
      var anno_id = select_anno.GetAnnoID();
      if (edit_popup_open) {
        StopTagEdit();
      ...
      if (idx != anno_id) {
        if (video_mode) oVP.HighLightFrames(LMgetTagField(LM_xml, idx, 't'), LMgettagField(LM_xml, idx, 'userlabeled'));
        ChangeLinkColorFG(idx);
        StartTagEdit(idx, null);
      }
    }
  };
  ...
  this.SubmitTag = function () {
    ...
    var tmp_id = $(LM_xml).children('annotation').children('tag').length + 1
